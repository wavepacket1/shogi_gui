...
1. **成り/不成りの状態遷移**:
   - 成れる駒（歩、香、桂、銀、角、飛）は、ダブルクリックにより以下の状態遷移を行います：
     - 通常駒 → 成駒 → 通常駒（循環）
   - 成れない駒（金、玉）は状態遷移を行いません

2. **先手/後手の状態遷移**:
   - すべての駒は、ダブルクリックにより以下の状態遷移を行います：
     - 先手駒 → 後手駒 → 先手駒（循環）
   - 成り/不成りの状態を保持したまま回転します
...
4. **状態遷移の制約**:
   - 成り/不成りの切り替えは、成れる駒のみ可能
   - 先手/後手の切り替えは、すべての駒で可能
   - 状態遷移は必ず上記の優先順位に従う

5. **状態の表現**:
   - 先手通常駒: 大文字（P, L, N, S, G, B, R, K）
   - 後手通常駒: 小文字（p, l, n, s, g, b, r, k）
   - 先手成駒: 大文字の前に'+'（+P, +L, +N, +S, +B, +R）
   - 後手成駒: 小文字の前に'+'（+p, +l, +n, +s, +b, +r）

この状態遷移の定義により、駒の操作が明確になり、実装時の一貫性が保たれます。また、ユーザーにとっても操作の予測が容易になります。
